...
public static class CustomerEndpoints
{
    ...
    public static void MapCustomerEndpoints(this RouteGroupBuilder api)
    {
        api.MapGet("/customers", async (StoreDb db) =>
        {
            ...
            var sales = await db.Sales.AsNoTracking().Where(x => x.CustomerId != null && x.PaymentMethod == "Credit")
            ...
            var balances = sales.GroupBy(x => x.CustomerId).ToDictionary(g => g.Key, g => g.Sum(x => Math.Max(0, x.Total - x.Returned - x.Paid)));
            return Results.Ok(customers.Select(x => new { x.Id, x.Name, x.Phone, x.Email, x.CreditLimit, x.IsActive,
                receivable = balances.GetValueOrDefault(x.Id) }));
        }).RequireAuthorization(StorePermissions.CustomersRead);

        api.MapPost("/customers", async (CustomerRequest input, StoreDb db) =>
        {
            if (!Valid(input)) return Results.BadRequest("Enter a customer name and a valid non-negative credit limit.");
            var customer = new Customer { Name = input.Name.Trim(), Phone = Clean(input.Phone), Email = Clean(input.Email), CreditLimit = input.CreditLimit };
        ...
        api.MapPut("/customers/{id:long}", async (long id, CustomerRequest input, StoreDb db) =>
        {
            ...
            customer.Name = input.Name.Trim(); customer.Phone = Clean(input.Phone); customer.Email = Clean(input.Email); customer.CreditLimit = input.CreditLimit;
            await db.SaveChangesAsync();
            return Results.Ok(new { customer.Id, customer.Name, customer.Phone, customer.Email, customer.CreditLimit, customer.IsActive });
        }).RequireAuthorization(StorePermissions.CustomersManage);
        ...
        api.MapGet("/customers/{id:long}/ledger", async (long id, StoreDb db) =>
        {
            var customer = await db.Customers.AsNoTracking().Where(x => x.Id == id)
                .Select(x => new { x.Id, x.Name, x.Phone, x.Email, x.CreditLimit, x.IsActive }).SingleOrDefaultAsync();
            if (customer is null) return Results.NotFound();
            var invoices = await db.Sales.AsNoTracking().Where(x => x.CustomerId == id && x.PaymentMethod == "Credit")
            ...
            return Results.Ok(new { customer, invoices, receivable = invoices.Sum(x => Math.Max(0, x.Total - x.returned - x.paid)) });
        }).RequireAuthorization(StorePermissions.CustomersRead);
        ...
        api.MapPost("/customers/{id:long}/payments", async (long id, CustomerPaymentRequest input, StoreDb db) =>
        {
            ...
            var sale = await db.Sales.SingleOrDefaultAsync(x => x.Id == input.SaleId && x.CustomerId == id && x.PaymentMethod == "Credit");
            if (sale is null) return Results.NotFound("Credit invoice not found for this customer.");
    ...
    private static bool Valid(CustomerRequest input) => !string.IsNullOrWhiteSpace(input.Name) && input.Name.Trim().Length <= 200 &&
        input.CreditLimit >= 0 && input.CreditLimit < 1_000_000_000m && (input.Phone?.Length ?? 0) <= 40 && (input.Email?.Length ?? 0) <= 254;
    private static string? Clean(string? value) => string.IsNullOrWhiteSpace(value) ? null : value.Trim();
}

public sealed record CustomerRequest(string Name, string? Phone, string? Email, decimal CreditLimit);
