Warning: truncated output (original token count: 17193)
  ...
  await page.route('**/api/**', async route => {
    ...
    if (medicineInventoryDetails) {
      ...
      const purchases = batches.map((batch, index) => ({ purchaseId: index + 1, purchaseLineId: index + 1,
        batchId: batch.id, batch: batch.number, supplier: 'Demo Pharma', supplierInvoice: `SUP-${index …7193 tokens truncated….click();
