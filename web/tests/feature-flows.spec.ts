Warning: truncated output (original token count: 19508)
...
async function mockApi(page: Page) {
  ...
  state.roles.push(
    ...
    { id: 'inventory-manager', name: 'Inventory Manager', permissions: ['medicines.read', 'medicines.manage', 'inventory.read', 'inventory.manage', 'purchases.read', 'purchases.create', 'purchases.manage', 'sales.read', 'suppliers.read', 'reports.read'], availablePermissions: state.permissionOptions.map(([key, label]) => ({ key, label })), canAssign: true },
  );
  ...
  await page.route('**/api/**', async route => {
    ...
    if (purchaseReturn && method === 'POST') {
      const purchase = state.purchases.find(x =…7508 tokens truncated…le('button', { name: 'Save purchase correction' }).click();
